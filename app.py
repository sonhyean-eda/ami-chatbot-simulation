...
# ------------------------------------------------------------
# 설계 원칙:
# - 활력징후, 검사결과, 의사 처방은 환자 응답이 아니라 시스템 정보로만 제시한다.
# - 환자는 통증, 호흡곤란, 불안, 식은땀 등 주관적 증상과 감정만 표현한다.
...
def update_exam_explanation_state(text: str) -> List[str]:
    ...
    if has_any(text, blood_keywords):
...
def update_intervention_explanation_state(text: str) -> List[str]:
    ...
    if has_any(text, oxygen_keywords):
...
def classify_input(user_text: str) -> str:
    ...
    # 활력징후 확인 후 학생의 안심/설명 발화
    # 예: "정상 범위보다 높지만 괜찮을 거예요"가 검사결과 확인(labs)으로 오분류되지 않도록 먼저 분류한다.
    vital_reassurance_keywords = [
        "정상 범위보다 높", "정상범위보다 높", "정상보다 높",
        "혈압이 높", "혈압은 높", "맥박이 빠르", "맥박은 빠르",
        "호흡수가 빠르", "산소포화도가 낮", "산소 포화도가 낮",
        "괜찮을 거예요", "괜찮을겁니다", "괜찮습니다", "괜찮아요",
        "걱정하지 마세요", "너무 걱정하지", "안정하세요", "안정하셔도",
        "제가 계속 확인하겠습니다", "계속 확인하겠습니다", "계속 관찰하겠습니다",
        "계속 살피겠습니다", "옆에 있겠습니다", "지켜보겠습니다"
    ]
    if (
    ...
    # 검사결과 확인/임상 판단
    # 주의: "정상범위", "높다", "괜찮다" 등 활력징후 설명 표현만으로는 검사결과 확인으로 분류하지 않는다.
    labs_keywords = [
...
def get_response(user_text: str) -> List[Dict[str, str]]:
    ...
    if category == "intro":
        ...
        responses.append(feedback_message(
            "활력징후 이상을 확인했으므로, 통증과 위험요인을 종합하여 AMI 가능성을 판단하고 필요한 검사를 설명해보세요."
        ))

    elif category == "labs":
        if not st.session_state.exam_explained:
            responses.append(patient_message(
                "선생님… 지금 제 상태를 확인해야 하는 건 알겠는데, 왜 필요한지 먼저 설명해주시면 조금 안심될 것 같아요…"
            ))
            responses.append(system_message(
                "검사결과\n"
...
# 프로그램 설명: 첫 화면 왼쪽 상단에는 간단한 프로그램 설명만 표시한다.
st.sidebar.info("""
🫀 **AMI 챗봇 가상환자 시뮬레이션**

응급실 학생간호사 역할로 급성심근경색 의심 환자를 사정하고,  
검사 설명, SBAR 보고, 중재 수행, 재사정을 연습하는 프로그램입니다.

King의 목표달성이론 흐름에 따라 단계적으로 진행됩니다.
""")
